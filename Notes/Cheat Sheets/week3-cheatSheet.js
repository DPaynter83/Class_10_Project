...
// 1) forEach: iterate (no return) ---> it does not return a ne array
//syntax: array.forEach(callBack())
const nums = [6, 4, 3, 2, 5, 1];
// nums.forEach(n => console.log(n));

// 2) map: transform → new array
// array.map(callback(currentVaule))
const doubled = nums.map(n => n * 2);
// console.log(doubled); // [12,8,6,4,10,2]

// 3) filter: keep items that pass condition
// creates a new array
// syntax: array.filter(callback(currentVaule));
const evens = nums.filter(n => n % 2 === 0);
// console.log(evens); // [6,4,2]

// 4) reduce: fold to a single value
// reduces the array to a single value
// used for sums, totals aggregations
//syntax: array.reduce(callBack(accmulator, currentValue)), intial value for your acc)
